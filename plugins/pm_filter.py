...
async def advantage_spell_chok(msg):
    ...
    if not movielist:
        button = InlineKeyboardMarkup([[
                 ...
                 k = await message.reply_photo(photo=imdb.get('poster'), caption=script.IMDB_MOVIE_2.format(mention=message.from_user.mention, query=search, title=imdb.get('title'), genres=imdb.get('genres'), year=imdb.get('year'), rating=imdb.get('rating'), url=imdb['url'], short=imdb.get['short_info']), reply_markup=reply_markup) 
